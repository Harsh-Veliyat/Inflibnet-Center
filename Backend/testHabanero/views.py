...
class GetDataForReact(APIView):
    def get(self,request):
        data =  collection_name.find({},{"container-title":1 ,"DOI":1, "author":1}).limit(5)
        dataList = []
        for d in data:
            d['_id'] = str(d['_id'])
            dataList.append(d)
...
# next_cursor = data[3]['message']['next-cursor']
# print(next_cursor)
# print('started')
# while skip != 0:
#     data = cr.works(filter={'has-affiliation':True, 'type':'journal-article'},cursor=next_cursor, select=['next-cursor'])
#     print(data)
#     print('skiped ',skip)
#     skip-=1
...
#data = cr.works( filter={'has-affiliation':True, 'type':'journal-article'}, cursor='*')#, cursor='*' query_affiliation='gandhinagar',

#data = data['message']
#data = data['DOI']
#collection_name.insert_one(data[3]['message'])
# data = cr.works(query_affiliation='gandhinagar', filter={'has-affiliation':True, 'type':'journal-article', 'from-pub-date':"2021"}, cursor='')#, cursor='*'
#collection_name.insert_one(data['message'])
#print(data[3],"\n\n",type(data[3]))
#print(data[3], file=open("output.txt", 'w', encoding='utf-8'))
#print(data[3]['message']['next-cursor'])
# for d in data:
#     print(d)
